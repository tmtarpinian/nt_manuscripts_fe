<template>
    <footer>
        <ul class="social">
                <Icon v-for="socialLink in socialLinksArray"
                :key="socialLink"
...
<script>
import SOCIAL_MEDIA_PLATFORMS from '../../modules/messages';
import Icon from '../svg/icons/Icon.vue';

export default {
  components: { Icon },
  data() {
    return {
      socialinks: SOCIAL_MEDIA_PLATFORMS,
    };
  },
  computed: {
    socialLinksArray() {
      return Object.values(this.socialinks);
    },
  },
};
</script>
...
footer{
    display:table; padding-bottom:30px; width:100%;
}
.social{
    margin:0px auto; display:table; display:table;
}
.social li{
